          ...
          <div key={footerLinks.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
            ...
            <ul className='list-none mt-4'>
              {footerLinks.links.map((links, index)=>(
                <li key={links.name} className={`font-popins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index!==footerLinks.links.length-1?'mb-4':'mb-0'}`}>
                  {links.name}
                </li>
              ))}
            </ul>
